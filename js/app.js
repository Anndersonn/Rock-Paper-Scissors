document.addEventListener('DOMContentLoaded', function () {
	...
	function winner(userCh, compCh) {
		...
		switch (comb) { //  сравнивает выражение со случаями, перечисленными внутри неё
			case 'pp': // определенные варианты, используется вместе со switch
			case 'ss':
			case 'rr':
			res.innerText = 'Draw!'; // ???
			sound.setAttribute('src', 'audio/draw.mp3');
			sound.play();
			break; // определенные варианты, используется вместе со switch
			...
			default: // определенные варианты, используется вместе со switch
			break;
		}
	}
